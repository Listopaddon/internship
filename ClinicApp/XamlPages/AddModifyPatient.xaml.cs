    ...
    public partial class AddModifyPatient : Page
    {
        ...
        private async void btnSubmit_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                if (dateOfBirth >= DateTime.Now)  //fix bag number 1. read file FixBags.docx
                {
                    MessageBox.Show("Введена не верная дата рождения");
                    return;
                }
                else if (dateOfBirth < new DateTime(1753,1,1))  //fix bag number 2. read file FixBags.docx
                {
                    MessageBox.Show("Введена не верная дата рождения");
                    return;
                }
